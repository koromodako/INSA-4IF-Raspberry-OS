...
void do_sys_yieldto(uint32_t * stack_pointer)
{
    // stack_pointer est sur le numéro d'appel système
    // On aura le paramètre à la position suivante
    struct pcb_s* dest = *((struct pcb_s**)(stack_pointer + 1));

    // Sauvegarde
    uint32_t * current_sp = stack_pointer;
    for (int i = 0; i < NB_SAVED_REGISTERS; ++i)
    {
        current_process->registres[i] = *current_sp;
        *current_sp = dest->registres[i];
        current_sp++;
    }
    current_process->lr_svc = *current_sp;
    *current_sp = dest->lr_user;
