...
for ((i = $ASCII_START; i <= $ASCII_END; i++)); do
	...
	cat ${TMP_DIR_PATH}/${TMP_DIR_NAME}/$i.xbm | grep -v '#define' | sed "s/static char /static char char_font_/" >> ${FONT_FILES_PATH}/${FONT_FILE_C_NAME} && \
